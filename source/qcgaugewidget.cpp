...
void QcBackgroundItem::draw(QPainter* painter)
{
    ...
    QRadialGradient linearGrad(tmpRect.center(), 2*position());
    linearGrad.setColorAt(0, Qt::white);
    linearGrad.setColorAt(0.2, QColor(65,131,215));
...
void QcColorBand::draw(QPainter *painter)
{
    ...
    for(int i = 0;i<mBandColors.size();i++){
        ...
        path = createSubBand(-offset,sweep);
        offset += sweep;
